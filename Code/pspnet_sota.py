...
for eachbackbone in backbones:
    for split_ratio in  train_ratio:
        for fold in np.arange(1):
            ...
            callbacks = [
                ModelCheckpoint('./pspnet_models/best_model_pspnet_'+str(eachbackbone)+'_'+str(fold)+'_'+str(split_ratio)+'.h5', save_weights_only=True, save_best_only=True, mode='min'),
                ReduceLROnPlateau(),
            ]
            ...
            history = model.fit(train_gen,steps_per_epoch=np.ceil(float(len(train_images)) / float(BATCH_SIZE)),
                                ...
                                callbacks = callbacks)
            np.save('./pspnet_models/pspnet_history_'+str(eachbackbone)+'_'+str(fold)+'_'+str(split_ratio)+'.npy',history.history)
            val_gen = generate_data(validation_images, BATCH_SIZE, (WIDTH, HEIGHT), val=True)
        ...
        break      
    df_metrics.to_csv('./pspnet_models/pspnet_history_'+str(eachbackbone)+'_'+str(split_ratio)+'.npy')
