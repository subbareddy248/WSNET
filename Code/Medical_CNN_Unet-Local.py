...
local_model = Unet(backbone_name='mobilenet',input_shape=(64, 64, 3),classes=1,activation='sigmoid', encoder_freeze=True)
...
X_patch1 = Lambda(putconcate)([out0, out1, out2])
X_patch2 = Lambda(putconcate)([out3, out4, out5 ])
X_patch3 = Lambda(putconcate)([out6, out7, out8])
...
X_patch = Lambda(putconcate_vert)([X_patch1, X_patch2, X_patch3])
...
callbacks = [
    ModelCheckpoint("./wacv/unet_local.h5", save_weights_only=True, save_best_only=True, mode='min')
]
model_1.compile(
    'Adam',
