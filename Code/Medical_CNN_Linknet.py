...
from segmentation_models import PSPNet, Unet, Linknet
...
# In[35]:
type_model_class4 = load_model('./woundtype_densenet_4classes_model.h5')

local_model = Linknet(backbone_name='mobilenet',input_shape=(64, 64, 3),classes=1,activation='sigmoid', encoder_freeze=False)
...
global_model = Linknet(backbone_name='mobilenet',input_shape=(192, 192, 3),classes=1,activation='sigmoid', encoder_freeze=False)
...
def generate_data(images_list, batch_size, dims, train=False, val=False, test=False):
        ...
        while True:
            ...
            for b in range(batch_size):
                ...
                i += 1
            if image_batch and image_batch1 and label_batch:
...
callbacks = [
    ModelCheckpoint("./wacv/linknet_wstech_imagenet1_nofreeze_densenet.h5", save_weights_only=True, save_best_only=True, mode='min')
]
model_1.compile(
    'Adam',
