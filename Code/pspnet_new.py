...
WIDTH = 384  # actual : 1918//1920 divisive by 64
HEIGHT = 384  # actual : 1280
BATCH_SIZE = 16
...
def generate_data_val(images_list, batch_size, dims, train=False, val=False, test=False):
        """Replaces Keras' native ImageDataGenerator."""
        try:
            if train is True:
                #print(images_list)
...
for eachbackbone in backbones:
    for split_ratio in  train_ratio:
        for fold in np.arange(1):
            ...
            val_gen = generate_data_val(validation_images, BATCH_SIZE, (WIDTH, HEIGHT), val=True)
            ...
            callbacks = [
                ModelCheckpoint('./pspnet_models/best_model_pspnet_aug_'+str(eachbackbone)+'_'+str(fold)+'_'+str(split_ratio)+'.h5', save_weights_only=True, save_best_only=True, mode='min'),
                ReduceLROnPlateau(),
            ]

            model.compile('Adam', dice_loss, ['binary_accuracy', dice_coef, sm.metrics.IOUScore(threshold=0.5), sm.metrics.FScore(threshold=0.5)])
            history = model.fit(train_gen,steps_per_epoch=np.ceil(float(8000) / float(BATCH_SIZE)),
                                epochs=100,
                                validation_steps=np.ceil(float(len(validation_images)) / float(BATCH_SIZE)),
                                validation_data = val_gen,
                                callbacks = callbacks)
            np.save('./pspnet_models/pspnet_aug_history_'+str(eachbackbone)+'_'+str(fold)+'_'+str(split_ratio)+'.npy',history.history)
        ...
        break      
    df_metrics.to_csv('./pspnet_models/pspnet_aug_history_'+str(eachbackbone)+'_'+str(split_ratio)+'.npy')
